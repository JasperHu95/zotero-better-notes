...
> **Fork notice:** This repository is a fork of [windingwind/zotero-better-notes](https://github.com/windingwind/zotero-better-notes), created to add custom improvements on top of the original plugin. All credit for the original work goes to [windingwind](https://github.com/windingwind).
...
- Download the plugin `.xpi` file from the [Releases page](https://github.com/JasperHu95/zotero-better-notes/releases):
  - [Latest Release](https://github.com/JasperHu95/zotero-better-notes/releases/latest) — includes the PDF chapter-headings feature.
...
1. **Magic Key** — Type `/` in the note editor to open the command palette,
   then select **Insert PDF Chapter Headings** (the label follows Zotero's UI
   language: _插入 PDF 章节标题_).
2. **Right-click menu** — Right-click anywhere in the note editor and choose
   **Insert PDF Chapter Headings**.
3. **Note template** — Call the helper functions directly:
   - `${await getPdfHeadings(topItem)}` — returns Markdown headings; pair with
     `// @use-markdown` to render them as formatted headings.
   - `${await getPdfOutline(topItem)}` — returns the raw outline tree for
